...
public class Tetris extends Canvas {
	...
	private final input.KeyboardInput keyboard = new input.KeyboardInput();
	private long lastIteration = System.currentTimeMillis();

	private static final int PIECE_WIDTH = 20;

	Boolean scorePasSauvegarde = true;
	...
	void tetrisLoop() {
		...
		if (keyboard.rotate()) {
			...
			game.drop();
		} else if (keyboard.dropSlow()) {
			game.moveDown();
		} else if (keyboard.savedPiece()) {

			game.savedPiece();
		} else if (keyboard.recoveredSavedPiece()) {
	...
	public void draw() throws IOException {
		...
		if (game.isPlaying()) {
			...
			if (game.isSaved()) {
				drawPieceSauvegardee(g, game.getPieceSauvegardee().getType());
		...
		if (game.isGameOver()) {
			...
			game.getListeScores().addScore(new Score("score", game.getTotalScore()));
			if (scorePasSauvegarde) {
				game.getListeScores().sauvegarderScores();
				scorePasSauvegarde = false;
			}
	...
	private void drawBestScores(Graphics2D g) {
		Font font = new Font("Dialog", Font.PLAIN, 16);
		g.setFont(font);
		g.setColor(Color.RED);
		g.drawString("1st : " + " " + game.getListeScores().getScore(1), 350, 550);
		g.drawString("2nd : " + " " + game.getListeScores().getScore(2), 350, 575);
		g.drawString("3rd : " + " " + game.getListeScores().getScore(3), 350, 600);
	}
	...
	private void drawHelpBox(Graphics2D g) {
		...
		g.drawString("", 10, 260);
		g.drawString("D: Drop SLOWLY", 10, 280);
		g.drawString("Q: Save a piece", 10, 300);
		g.drawString("W: Use the saved piece", 10, 320);
	}
	...
	private void drawPieceSauvegardee(Graphics2D g, PieceType type) {
		g.setFont(new Font("Dialog", Font.PLAIN, 16));
		g.setColor(Color.RED);
		g.drawString("Save:", 600, 420);
		for (Point p : type.getPoints()) {
			drawBlock(g, 610 + p.x * PIECE_WIDTH, 380 + (3 - p.y) * 20, getPieceColor(type));
		}
